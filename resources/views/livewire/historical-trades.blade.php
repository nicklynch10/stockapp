    ...
    <table class="historical">
        ...
        @forelse($tradesdata as $trad)
            ...
            <tr class="mt:flex mt:flex-col mt:border-2-solid-black mt:border-r-11 mt:mb-2">
                ...
                <td data-label="Summary" class="p-3 whitespace-nowrap text-center text-gray-900 mt:grid mt:text-xs mt:text-left mt:py-1 mt:px-3"><p class="whitespace-normal">{!! $trad->type==0?'Purchased '.round($trad->stock, 2).' '.($trad->stock > 9 ? "Shares" : "Share" ).' of '.$trad->stock_ticker.' on '.\Carbon\Carbon::createFromTimestamp(strtotime($trad->date_of_transaction))->format('F jS, Y').' for $'.number_format($trad->share_price,2).' per share':'Sold '.$trad->stock.' shares of '.$trad->stock_ticker.' on '.\Carbon\Carbon::createFromTimestamp(strtotime($trad->date_of_transaction))->format('F jS, Y').' for $'.number_format($trad->share_price,2).' per share'  !!}</p></td>
