    ...
    <table>
        ...
        @forelse($tradesdata as $trad)
            ...
            <tr>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-center text-gray-900">{{ $trad->type==0?'Bought '.$trad->stock.' shares of '.$trad->stock_ticker.' on '.\Carbon\Carbon::createFromTimestamp(strtotime($trad->date_of_transaction))->format('F dS, Y').' for $'.number_format($trad->share_price,2).' per share':'Sold '.$trad->stock.' shares of '.$trad->stock_ticker.' on '.\Carbon\Carbon::createFromTimestamp(strtotime($trad->date_of_transaction))->format('F dS, Y').' for $'.number_format($trad->share_price,2).' per share' }}</td>
