                        ...
                        <div class="w-full mb-5 overflow-hidden" style="height: 100%">
                            <div class="grid border-b-2 border-gray-300 grid-cols-8 xs:grid-cols-1 sm:grid-cols-1 md:grid-cols-4 xl:grid-cols-8 lg:grid-cols-8 p-2 overflow-y-auto overflow-x-hidden  w-2/4w-full " style="max-height: 65vh;">
                                <div class="col-start-1 col-span-1 sm:flex sm:flex-col sm:col-stat-1 md:col-start-1 flex justify-between items-center w-full">
                                    <h2 class="text-xl font-black xs:mb-4">Your Portfolio</h2>
                                </div>
                                <div
                                    class="lg:col-start-7 xl:col-start-7 md:col-start-3 col-span-1 xs:col-start-1 inline-flex items-center xs:justify-left">
                                    <select  wire:model="sortBy" class="shadow appearance-none border w-60 xs:w-44 rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                                ...
                                <div class="lg:col-start-8 xl:col-start-8  md:col-start-4 col-span-1 xs:col-start-1 sm:text-center lg:text-center xl:text-center md:text-center ">
                                    <x-jet-button wire:click="create()" class="py-2 px-3 my-3" id="add">{{__('Buy New Stock') }}</x-jet-button>
                                </div>
                            </div>

                            <div class="grid grid-cols-8 xs:grid-cols-1 sm:grid-cols-1 md:grid-cols-4 lg:grid-cols-8 p-2 overflow-y-auto overflow-x-hidden w-2/4w-full xs:hidden" style="max-height: 65vh;">
                                @if(count($stocks)>0)
                                    @foreach($stocks as $s)
                            ...
                            <div class="grid grid-cols-12 xl:hidden lg:hidden md:hidden sm:hidden mt-4">
                                @if(count($stocks)>0)
                                    @foreach($stocks as $s)
                                        @if(round($s->share_number,2)!=0 && round($s->share_number,2)!=0.00)
                                            @php
                                            ...
                                            <div class="col-span-2 col-start-1">
                                                <div class="float-left">
                                                    ...
                                                    ?>
                                                    @if(isset($logourl))
                                                        <img src="{{ $logoUrl }}" class="rounded-full object-contain hover:bg-gray-100">
                                                    @else
                                                        @php
                                                            $count= strlen($s->stock_ticker)
                                                        @endphp
                                                        <div class="text-xs rounded-full border-gray-300 bg-blue-50 flex items-center font-bold text-blue-500 justify-center w-12 h-12 flex-shrink-0 mx-auto">
                                                            <span class="break-all">{{ strtoupper($s->stock_ticker) }}</span>
                                                        </div>
                                                    @endif
                                                </div>
                                            ...
                                            <div class="col-span-5 col-start-3">
                                                <a class="break-words cursor-pointer pb-2 text-black font-black hover:bg-gray-100 xs:text-sm bold font-sans hover:bg-gray-100" wire:click="company({{ $s->id }})">
                                                    {{ strtoupper($s->stock_ticker) }}
                                                </a>
                                                <p class="pb-2 text-sm font-sans break-words font-light text-grey-dark italic sm:text-xs">
                                                    @if($s->security_name != null && convertType($s->issuetype) == "ETF")
                                                ...
                                                <p class="pb-2 text-sm font-sans break-words font-light text-grey-dark italic sm:text-xs justify-center">
                                                    {{ convertType($s->issuetype) }}
                                                </p>
                                            </div>
                                            <div class="col-span-2 col-start-9">
                                                <p class="pb-1 text-sm font-sans break-words font-light text-green-600 float-right">
                                                    ${{ number_format($s->ave_cost * $s->share_number,2)}}
                                                    {{--                                                            ${{ number_format((int)$s->ave_cost,2) * round((int)$s->share_number, 2) }}--}}
                                                </p>
                                            ...
                                            <div class="col-span-2 col-start-12 justify-center">
                                                <div class="absolute " x-data="{ open: false }" @click.away="open = false" @close.stop="open = false">
                                                    <div @click="open = ! open">
                                                        <span
                                                            class="inline-flex rounded-md text-gray-500 opacity-80 hover:opacity-100 cursor-pointer text-sm">
                                                            <i class="fa fa-ellipsis-v ml-2 -mr-0.5 h-4 w-4"></i>
                                            ...
                                            <div class="col-start-1 col-span-12 border-b border-gray-300 my-3"></div>
                                        @endif
                                    @endforeach
                                @else
                                    <div class="pt-3 text-lg text-center text-black"><i class="fa fa-chart-line"></i> No Stock Found</div>
